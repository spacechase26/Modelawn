...
git push fork modes-dev                  # triggers CI (ci.yml builds assembleLawnWithQuickstepGithubDebug)
...
- **Rebasing onto a newer Lawnchair (`16-dev` / `17-dev`):** the risk areas are the 3 Java
  hooks (`Launcher.java`, `Folder.java`, `FolderIcon.java`) and the gesture-config /
  preference-navigation files, since upstream refactors those. The `modes` package itself is
  self-contained and rarely conflicts. After rebasing, re-verify the four bind points still
  exist (search for `ModeWorkspaceGating`).
